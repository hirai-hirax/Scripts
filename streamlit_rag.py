...
st.set_page_config(page_title="ローカル風RAGアプリ", layout="wide")
st.title("ローカル風RAGアプリ")
...
uploaded_files = st.sidebar.file_uploader(
        "ファイルをアップロード",
        type=['pdf', 'docx', 'txt', 'xlsx', 'pptx', 'boxnote', 'msg'],
...
if uploaded_files:
    ...
    for uploaded in uploaded_files:
        ...
        for idx, i in enumerate(range(0, len(text), chunk_size)):
            chunk = text[i:i + chunk_size]
            all_docs.append(Document(page_content=chunk, metadata={"source": str(out_path), "position": idx}))
    ...
    vs.save_local(str(STORE_DIR))
    st.sidebar.success("アップロードファイルをまとめてベクトルDBに登録しました")
    st.rerun()
